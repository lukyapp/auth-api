...
import { InvalidArgumentsException } from '../../poppinss/exception/exception';
import {
  AbstractConstructor,
  Constructor,
  ExtractFunctions,
...
import { enqueue, isClass } from './helpers';
import { ContainerResolver } from './resolver';
import { ContextBindingsBuilder } from './contextual_bindings_builder';
...
export class Container<KnownBindings extends Record<any, any>> {
  ...
  make<Binding>(
    ...
    return this.createResolver().make<Binding>(
      binding,
      runtimeValues,
      createError,
    );
  ...
  call<Value extends Record<any, any>, Method extends ExtractFunctions<Value>>(
    ...
    return this.createResolver().call(
      value,
      method,
      runtimeValues,
      createError,
